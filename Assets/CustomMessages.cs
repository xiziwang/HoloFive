...
public class CustomMessages : Singleton<CustomMessages>
{
    ...
    public void SendHeadTransform(Vector3 position, Quaternion rotation, byte HasAnchor)
    {
        ...
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            ...
            Debug.Log("+++ +++ +++ +++ Head Transform Sent");

        }
    }

    public void SendHandTransform(Vector3 position, Quaternion rotation, byte HasAnchor)
    {
        // If we are connected to a session, broadcast our hand info
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            // Create an outgoing network message to contain all the info we want to send
            ...
            this.serverConnection.Broadcast(
                ...
                MessageReliability.UnreliableSequenced,
                MessageChannel.Avatar);
            Debug.Log("+++ +++ +++ +++ Hand Transform Sent");
        }
    }

    public void SendHandStatus(int status)
    {
        // If we are connected to a session, broadcast our head info
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            // Create an outgoing network message to contain all the info we want to send
            NetworkOutMessage msg = CreateMessage((byte)TestMessageID.HandStatus);
